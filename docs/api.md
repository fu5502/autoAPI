...
签到与余额刷新使用站点级总超时：设置中的 `siteTimeoutSeconds`（默认 15 秒，UI 可调 5-120 秒）覆盖同一站点签到、余额刷新及其 fallback 的整个循环，不再为 fallback 重新分配一个完整超时。超时或手动终止任务时会关闭当前页面并强制结束浏览器进程；普通授权取消优先走优雅关闭，让 Cookie/会话正常落盘。
