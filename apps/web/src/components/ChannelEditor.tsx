    ...
    <div className="drawer-layer open">
      ...
      <aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="channel-editor-title">
        ...
        <form onSubmit={submit}>
          ...
          <div className="field"><label htmlFor="edit-favicon-url">自定义渠道图标 <small>留空则自动获取，支持 HTTPS 或 Data URL</small></label><input id="edit-favicon-url" type="text" inputMode="url" autoCapitalize="none" spellCheck={false} value={faviconUrl} onChange={(event) => setFaviconUrl(event.target.value)} placeholder="https://example.com/icon.png 或 data:image/svg+xml,..." /></div>
