import { CheckCircle2, CircleAlert, Clock3, Coins, Layers3, MessageSquareText, X } from "lucide-react";
    ...
    <div className="modal-layer open">
      ...
      <section className="modal probe-result-modal" role="dialog" aria-modal="true" aria-labelledby="probe-result-title">
        ...
        <div className="probe-result-content">
          ...
          {probe.chatOk ? <div className="probe-reply"><div className="probe-reply-head"><MessageSquareText size={14} /><span>对话测试</span>{probe.probedModel ? <code>{probe.probedModel}</code> : null}</div><p className="probe-reply-prompt">请用一句话说明你是谁</p><p className="probe-reply-answer">{probe.probeReply?.trim() ? probe.probeReply : "上游返回成功，但未包含可显示的文本内容。"}</p></div> : null}
