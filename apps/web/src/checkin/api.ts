...
export const api = {
  ...
  runCheckin: (siteIds?: number[], operation: 'checkin' | 'balance_refresh' = 'checkin') => request<CheckinRun | null>('/admin/checkin/checkin/run', { method: 'POST', body: JSON.stringify({ ...(siteIds ? { siteIds } : {}), operation }) }),
  ...
  cancelRunSite: (runId: number, siteId: number) => request<CheckinRun>(`/admin/checkin/runs/${runId}/sites/${siteId}/cancel`, { method: 'POST', body: '{}' }),
