...
export default function CheckinModule({ view = 'dashboard' }: { view?: CheckinView }) {
  ...
  const runCheckin = async (siteIds?: number[]) => {
    ...
    try {
      const run = await api.runCheckin(siteIds)
      if (run?.id) {
        ...
        <span className="run-progress-popover-lines">
          {visible.length ? visible.slice(-60).map((entry) => (
            <span className={`run-progress-line ${entry.level}`} key={entry.id}>
              ...
              <span>{entry.siteName ? `${entry.siteName} · ` : ''}{entry.message}</span>
            </span>
  ...
  const renderSiteTable = (group: SiteManagementGroup, groupSites: Site[]) => {
    ...
    return <div className="table-wrap management-table-wrap">
      <table className="data-table management-table" aria-label={`${balanceOnly ? '中转站' : '公益站'}列表`}>
        <thead><tr><th>站点</th><th>适配器</th><th>登录</th><th>{balanceOnly ? '余额状态' : '签到状态'}</th><th>{balanceOnly ? '余额刷新时间' : '签到奖励 / 时间'}</th><th>当前余额</th><th>加入渠道</th><th>{balanceOnly ? '自动刷新' : '自动签到'}</th><th>操作</th></tr></thead>
        <tbody>{groupSites.length ? groupSites.map((site) => <tr key={site.id} data-site-group={group}>
          ...
          <td>{balanceOnly
            ...
            : <div className="reward-cell static" data-interactive="false" onClick={(event) => event.stopPropagation()}><strong className={rewardTimingTone(site.lastRewardAt)}>{formatAmount(site.lastRewardAmount, site.currencySymbol)}</strong><small>{site.lastRewardAt ? rewardTimingLabel(site.lastRewardAt) : '--'}</small></div>}</td>
